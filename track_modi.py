...
input_pts = np.array(coord_data["ori_points"], np.int32)
output_pts= np.array(coord_data["trans_points"], np.int32)
...
def run(
        source='0',
        yolo_weights='crowdhuman_yolov5m.pt',  # model.pt path(s),
    ...
    player_dict=[{"id":None,"prev_rect":None,"current_rect":None,"map":None}]*num_player
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if save_vid or save_crop or show_vid:  # Add bbox to image
                            ...
                            im0=cv2.circle(im0, (int(bbox_left)+int(bbox_w/2),int(bbox_top+bbox_h)), radius=20, color=(0, 255, 255), thickness=-1)
